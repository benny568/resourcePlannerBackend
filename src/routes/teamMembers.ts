...
import { TeamMemberData, PersonalHolidayData, ApiResponse, ApiError } from '../types';
...
router.get('/', async (req, res) => {
  try {
    ...
    const response: ApiResponse<typeof teamMembers> = {
      data: teamMembers
    };
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const response: ApiResponse<typeof teamMember> = {
      data: teamMember
    };
...
router.post('/', async (req, res) => {
  try {
    const { name, capacity, skills }: TeamMemberData = req.body;
...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { name, capacity, skills }: Partial<TeamMemberData> = req.body;
...
router.post('/:id/holidays', async (req, res) => {
  try {
    const { id } = req.params;
    const { startDate, endDate, description }: Omit<PersonalHolidayData, 'teamMemberId'> = req.body;
...
router.delete('/:id/holidays/:holidayId', async (req, res) => {
  try {
    ...
    const holiday = await prisma.personalHoliday.findFirst({
      where: {
        id: holidayId,
        teamMemberId: id
      }
    ...
    await prisma.personalHoliday.delete({
      where: { id: holidayId }
    });
