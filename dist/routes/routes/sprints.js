...
router.post('/batch', async (req, res) => {
    ...
    try {
        ...
        // DEBUG: Log sprint names to check for duplicates in input data
        const sprintNames = sprints.map(s => s.name);
        const uniqueNames = [...new Set(sprintNames)];
        if (sprintNames.length !== uniqueNames.length) {
            console.error(`🚨 DUPLICATE SPRINTS IN INPUT DATA!`);
            console.error(`  Total: ${sprintNames.length}, Unique: ${uniqueNames.length}`);
            console.error(`  Sprint names:`, sprintNames);
            const duplicates = sprintNames.filter((name, index) => sprintNames.indexOf(name) !== index);
                ...
                if (isRegeneration) {
                    ...
                    if (!existingSprint) {
                        ...
                        if (similarSprints.length > 0) {
                            console.warn(`⚠️ Found ${similarSprints.length} similar sprints for "${name}":`, similarSprints.map(s => `${s.name} (ID: ${s.id})`));
