...
const ProjectKeySchema = zod_1.z.object({
    projectKey: zod_1.z.string().min(1).max(10).default('REF')
});
// Schema for paginated epic import
const PaginatedProjectKeySchema = zod_1.z.object({
    projectKey: zod_1.z.string().min(1).max(10).default('REF'),
...
function convertJiraStatusToWorkItemStatus(jiraStatus) {
...
function validateStoryPoints(value) {
...
function extractTextFromADF(adfContent) {
    ...
    if (adfContent.content && Array.isArray(adfContent.content)) {
        return adfContent.content.map((node) => extractTextFromADF(node)).join('\n');
    }
...
async function callJiraAPI(endpoint, body) {
    ...
    try {
        ...
        if (endpoint === 'search') {
            ...
            if (body.fields)
                params.append('fields', body.fields);
            if (body.expand)
                params.append('expand', body.expand);
...
router.post('/ticket', async (req, res) => {
    try {
        ...
        const workItem = {
            ...
            estimateStoryPoints: validateStoryPoints(jiraTicket.fields[storyPointsField]),
            requiredCompletionDate: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString(), // 90 days from now
            requiredSkills: (jiraTicket.fields.labels || []).filter(label => ['frontend', 'backend', 'devops', 'design', 'qa'].includes(label.toLowerCase())),
            dependencies: [],
...
router.post('/work-items', async (req, res) => {
    try {
        ...
        if (includeVelocity) {
            ...
            if (completedData.issues.length > 0) {
                console.log(`🔍 DEBUG: First completed issue:`, {
                    ...
                    storyPoints: completedData.issues[0].fields[storyPointsField],
            ...
            completedData.issues.forEach(issue => {
                const points = issue.fields[storyPointsField] || 1;
        ...
        const workItems = (searchData.issues || []).map(ticket => ({
            ...
            estimateStoryPoints: validateStoryPoints(ticket.fields[storyPointsField]),
            requiredCompletionDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString(), // 60 days for regular work items
            requiredSkills: (ticket.fields.labels || []).filter(label => ['frontend', 'backend', 'devops', 'design', 'qa'].includes(label.toLowerCase())),
            dependencies: [],
...
router.post('/epics', async (req, res) => {
    try {
        ...
        const workItems = (searchData.issues || []).map(epic => ({
            ...
            estimateStoryPoints: validateStoryPoints(epic.fields[storyPointsField]),
            requiredCompletionDate: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toISOString(), // 180 days for epics
            requiredSkills: (epic.fields.labels || []).filter(label => ['frontend', 'backend', 'devops', 'design', 'qa'].includes(label.toLowerCase())),
            dependencies: [],
...
router.post('/epics-with-children', async (req, res) => {
    ...
    try {
        ...
        const processEpics = async () => {
            ...
            const processEpicWithChildren = async (epic) => {
                const epicData = {
                    id: epic.key,
                    jiraId: epic.key,
                    title: epic.fields.summary,
                ...
                try {
                    ...
                    for (const child of childrenData.issues || []) {
                        const rawStoryPoints = child.fields[storyPointsField];
            ...
            for (let i = 0; i < epics.length; i += batchSize) {
                ...
                const batchResults = await Promise.all(batch.map(epic => processEpicWithChildren(epic)));
        ...
        const result = await Promise.race([processEpics(), timeoutPromise]);
...
router.post('/ai-import-epics', async (req, res) => {
    try {
        ...
        const processedEpics = epicsData.map((epic) => ({
            id: epic.id || epic.key,
            jiraId: epic.key,
        ...
        const totalChildren = processedEpics.reduce((total, epic) => total + (epic.children?.length || 0), 0);
        const totalStoryPoints = processedEpics.reduce((total, epic) => total + (epic.totalStoryPoints || 0), 0);
...
async function getCompletedSprintsFromJira(projectKey, sprintNames) {
    try {
        ...
        completedIssuesResponse.issues.forEach(issue => {
            ...
            if (versions.length > 0) {
                ...
                if (sprintNames && sprintNames.length > 0) {
                    const matchesFilter = sprintNames.some((name) => sprintKey.toLowerCase().includes(name.toLowerCase()));
                    if (!matchesFilter)
                        return;
                }
...
async function syncSprintData(jiraSprint) {
    try {
        ...
        let existingSprint = await prisma_1.prisma.sprint.findFirst({
            where: {
        ...
        if (!existingSprint) {
            const fuzzySprints = await prisma_1.prisma.sprint.findMany({
                where: {
                    name: {
            ...
            if (fuzzySprints.length > 1) {
                console.warn(`⚠️ Multiple sprints found for fuzzy match "${jiraSprint.name}":`, fuzzySprints.map(s => `${s.name} (ID: ${s.id})`));
        ...
        await prisma_1.prisma.sprint.update({
        ...
        for (const assignment of existingSprint.workItemAssignments) {
            ...
            if (workItem.jiraId) {
                ...
                const completedInJira = jiraSprint.completedIssues.some((issue) => issue.key === workItem.jiraId);
                if (completedInJira && workItem.status !== 'Completed') {
                    // Update work item status to completed
                    await prisma_1.prisma.workItem.update({
...
router.post('/sync-past-sprints', async (req, res) => {
    try {
        ...
        const pastSprints = await prisma_1.prisma.sprint.findMany({
            where: {
                archived: false,
...
function extractSprintName(sprintData) {
    try {
...
async function syncCompletedTicketToSprint(jiraIssue, pastSprints, storyPointsField) {
    const ticketKey = jiraIssue.key;
    const summary = jiraIssue.fields.summary;
    const points = jiraIssue.fields[storyPointsField] || 1;
    ...
    if (sprints && sprints.length > 0) {
        ...
        if (sprintName) {
            targetSprint = pastSprints.find(sprint => sprint.name.toLowerCase() === sprintName.toLowerCase() ||
    ...
    let workItem = await prisma_1.prisma.workItem.findFirst({
        where: { jiraId: ticketKey }
    });
    if (!workItem) {
        // Create new work item from Jira ticket
        const skills = (jiraIssue.fields.labels || []).filter((label) => ['frontend', 'backend', 'devops', 'design', 'qa'].includes(label.toLowerCase()));
        ...
        workItem = await prisma_1.prisma.workItem.create({
            data: {
                jiraId: ticketKey,
                title: summary,
    ...
    if (!existingAssignment) {
        ...
        await prisma_1.prisma.sprintWorkItem.create({
            data: {
                sprintId: targetSprint.id,
                workItemId: workItem.id
            }
...
async function updateSprintVelocities(sprints) {
    const updates = [];
    for (const sprint of sprints) {
        // Calculate actual velocity from completed work items assigned to this sprint
        const completedWorkItems = await prisma_1.prisma.sprintWorkItem.findMany({
            where: {
                sprintId: sprint.id,
                workItem: {
                    status: 'Completed'
